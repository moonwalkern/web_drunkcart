...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'ajax' => "editProduct(". $result['product_id'] . ")"
			);
	...
	protected function getForm() {
        ...
        echo print_R($this->data['profiles'],TRUE);
		...
		if (!empty($this->request->post)) {
			...
			foreach ($this->request->post as $key => $value) {
				if (strpos($key, 'filter') !== 0) {
					$product = explode("~", $key);
					if (strpos($key, 'quantity') === 0) {///This is stupid logic, need to remove.
						$data['quantity'] = $value;	
					}elseif(strpos($key, 'status') === 0){
						$data['status'] = $value;
						$product_id = $product[1];
						$this->model_catalog_product->editProductAttributes($product_id, $data);	
					}
				}
        	...
        	$json['success'] = $this->language->get('text_update_message');
		}
