	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				*/
				if (json['error']['states']) {
					$('#payment-address select[name=\'states_id\'] + br').after('<span class="error">' + json['error']['states'] + '</span>');
				}	
				
				if (json['error']['locality']) {
					$('#payment-address select[name=\'locality_id\'] + br').after('<span class="error">' + json['error']['locality'] + '</span>');
				}
