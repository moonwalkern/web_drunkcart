...
var oprand = {
    ...
    FileReaderJS.setupInput(document.getElementById('imageFile'), oprand); 
...
create_box = function(e,file){
	...
	var template = '<div class="eachImage" id="div'+rand+'">';
	template += '<span class="preview" id="'+rand+'"><img src="'+src+'"><span class="overlay"><span class="updone"></span></span>';
	template += '</span>';
    template += '<input class="button" type="button" id="'+file.name+'" name="'+rand+'" value="remove" onClick="removeImage(this)">';
...
upload = function(file,rand){
    ...
    //alert(ajax_url); 
	xhr[rand].open("post", ajax_url, true);
	  ...
	  if (xhr[rand].readyState === 4) {  
		if (xhr[rand].status === 200) {  
          ...
          if(document.getElementById('images').value == ""){
            document.getElementById('images').value = file.name ;
          }
          else{
            document.getElementById('images').value = document.getElementById('images').value + "," + file.name;
          }
		} else {  
    ...
    xhr[rand].setRequestHeader("X-File-Path", "//Users//sreejigopal//Documents//DrunkCart.com//MyDrunkCart//image");
    //xhr[rand].setRequestHeader("X-File-Path", "C://Sreeji//opencart//image//");
