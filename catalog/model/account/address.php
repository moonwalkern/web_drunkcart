...
class ModelAccountAddress extends Model {
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			$locality_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "locality` WHERE locality_id = '" . (int)$address_query->row['locality_id'] . "'");
			if ($locality_query->num_rows) {
				$locality = $locality_query->row['locality_name'];
				$locality_id = $locality_query->row['locality_id'];
				$state_id = $locality_query->row['state_id'];
			} else {
				$locality = '';
				$locality_id = '';
			}	
			
			$zone_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone` WHERE zone_id = '" . (int)$state_id . "'");
			if ($zone_query->num_rows) {
				$zone = $zone_query->row['name'];
				$zone_code = $zone_query->row['code'];
			} else {
	...
	public function getAddresses() {
		...
		foreach ($query->rows as $result) {
			...
			$locality_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "locality` WHERE locality_id = '" . (int)$result['locality_id'] . "'");
			if ($locality_query->num_rows) {
				$locality = $locality_query->row['locality_name'];
				$locality_id = $locality_query->row['locality_id'];
				$state_id = $locality_query->row['state_id'];
