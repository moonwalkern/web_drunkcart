<?php 
class ControllerAccountAdList extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/adlist', '', 'SSL');

			$this->redirect($this->url->link('account/login', '', 'SSL'));
		}

		$this->language->load('account/adlist');
		...
		if (isset($this->request->get['remove'])) {
			...
			$this->redirect($this->url->link('account/adlist'));
		}
        ...
        $productCustomer = $this->model_catalog_product->getCustomerProduct($this->customer->getId());
        
        
        
		foreach ($productCustomer as $key => $product_id) {
			...
			if ($product_info) { 
				...
				$this->data['products'][] = array(
					...
					'date'       => date( 'd/M/Y', strtotime($product_info['date_added'])),
					'price'      => $price,		
					'special'    => $special,
					'href'       => $this->url->link('product/product', 'product_id=' . $product_info['product_id']),
					'edit'     => $this->url->link('product/postad/edit', '&id=' . $product_info['product_id']),
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/adlist.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/adlist.tpl';
		} else {
			$this->template = 'default/template/account/adlist.tpl';
		}
