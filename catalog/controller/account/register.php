...
class ControllerAccountRegister extends Controller {
	...
	protected function validate() {
		...
		$country_info = $this->model_localisation_country->getCountry(99);
        ...
        if (!isset($this->request->post['states_id']) || $this->request->post['states_id'] == '') {
			$this->error['states_id'] = $this->language->get('error_state');
		}
        
        if (!isset($this->request->post['locality_id']) || $this->request->post['locality_id'] == '') {
			$this->error['locality_id'] = $this->language->get('error_locality');
		}
    ...
    public function buildemaildata($data){
        ...
        $data['mobile'] = $this->request->post['telephone']; //mobile of the interested buyer/seller
    ...
    public function activateuser(){
        ...
        $activate = $this->model_account_customer->activateCustomerFromEmail($this->request->get);
        
        $this->data['warning'] = $this->language->get('error_notactive');
        $this->data['success'] = $this->language->get('text_success');
        
        if ($activate) {
        	if($this->request->get['checkout'] == '1'){
        		$this->redirect($this->url->link('checkout/checkout', 'success='.$this->data['success'].'&UUID='.$this->request->get['u'] ));
        	}else{
        		$this->redirect($this->url->link('account/loginactivate', 'success='.$this->data['success'] ));	
        	}
		}else{
			if($this->request->get['checkout'] == '1'){
				$this->redirect($this->url->link('checkout/checkout', 'error='.$this->data['warning'].'&UUID='.$this->request->get['u'] ));
        	}else{
				$this->redirect($this->url->link('account/loginactivate', 'error='.$this->data['warning'] ));	
        	}
		}
