	...
	public function index() { 
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			if(isset($this->session->data['location_id'])){
				...
				$this->session->data['location_id'] = 1647;
				$location_id = 1647;
			}
            ...
            //Check if the request came from device (iOS or Andriod)
			//If request is from device :iOS, the post data [device] will have iOS and Andriod
			if(isset($this->request->get['device'])){
				if($this->request->get['device'] == 'iOS'){
					$this->load->library('log');
	        		$logger = new Log('swapdeal.log');
					$logger->write('Post Data' . $this->request->get['device']);
					$logger->write('Device is iOS');
					if ($this->data['products']) {
						$json['data'] = $this->data['products'];
					}else{
						$json['data'] = 'No Data';
					}	
