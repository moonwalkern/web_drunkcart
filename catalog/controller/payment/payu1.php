		...
		if (isset($this->request->post['key']) && ($this->request->post['key'] == $this->config->get('payu_merchant'))) {
			...
			$this->data['text_failure_wait'] = sprintf($this->language->get('text_failure_wait'), $this->url->link('checkout/cart'));
			 ...
			 if (isset($this->request->post['status']) && $this->request->post['status'] == 'success') {
                ...
                $logger->write('salt-:'.$salt);
                $logger->write('saltString-:'.$saltString);
                $logger->write('reverseKeyString-:'.$reverseKeyString);
                ...
                $logger->write('post-:'.$this->request->post['hash']);
					if($sentHashString==$this->request->post['hash']){
							...
							$logger->write('rendering the success page');
							$this->children = array(
								'common/column_left',
								'common/column_right',
								'common/content_top',
						...
						//Transaction will be pending
                            ...
                            $logger->write('rendering the pending page '.$this->template);	
							$this->children = array(
								'common/column_left',
								'common/column_right',
								'common/content_top',
