...
class ModelUserUser extends Model {
	...
	public function editUser($user_id, $data) {
		...
		$localityArray = explode(",",$data['locality_id'] );
		$data['locality_id'] = $localityArray[0];
		$data['city'] = $localityArray[1];
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', mobile = '" .$data['mobile']  . "', phone = '" . $data['phone'] . "', user_group_id = '" . (int)$data['user_group_id'] . "', status = '" . (int)$data['status'] . "' WHERE user_id = '" . (int)$user_id . "'");
		
		$this->db->query("UPDATE `" . DB_PREFIX . "address` SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', address_1 = '" . $this->db->escape($data['address_1']) . "', address_2 = '" . $this->db->escape($data['address_2']) . "', city = '" . $this->db->escape($data['city']) . "', states_id = '" . (int)$data['states_id'] . "', locality_id = '" . (int)$data['locality_id'] . "' WHERE customer_id = '" . (int)$user_id . "'");
	...
	public function deleteUser($user_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "address` WHERE customer_id = '" . (int)$user_id . "'");
	}

	public function getUser($user_id) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "user` u";
		$sql .= " LEFT OUTER JOIN `" . DB_PREFIX . "address` a ON u.user_id = a.customer_id";	
	...
	public function getUserByUsername($username) {
		...
		$sql = "SELECT * FROM `" . DB_PREFIX . "user` u";
		$sql .= " LEFT OUTER JOIN `" . DB_PREFIX . "address` a ON u.user_id = a.customer_id";
		$sql .= " WHERE username = '". $this->db->escape($username) . "'";	
		$query = $this->db->query($sql);
		return $query->row;
	}
	...
	public function getUserByCode($code) {
		...
		$sql = "SELECT * FROM `" . DB_PREFIX . "user` u";
		$sql .= " LEFT OUTER JOIN `" . DB_PREFIX . "address` a ON u.user_id = a.customer_id";
		$sql .= " WHERE code = '" . $this->db->escape($code) . "' AND code != ''";	
		$query = $this->db->query($sql);
		return $query->row;
	}
