...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['vendor_id'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_vendor SET `product_id` = " . (int)$product_id . ", vendor_id = " . (int)$data['vendor_id']);
		}
	...
	public function copyProductWithVendor($product_id, $vendor_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getProductsByVendor($vendor_id) {
		return $this->db->query("SELECT product_id FROM `" . DB_PREFIX . "product_to_vendor` WHERE vendor_id = " . (int)$vendor_id)->rows;
	}
