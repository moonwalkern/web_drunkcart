...
class ControllerUserUser extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$vendor_id = $this->model_user_user->addUser($this->request->post);
			
			$this->load->model('catalog/product');
			$masterProduct = $this->model_catalog_product->getProductsByVendor(1);
			foreach ($masterProduct as $product_id) {
				$this->model_catalog_product->copyProductWithVendor($product_id['product_id'], $vendor_id);
			}
	...
	protected function getForm() {
		...
		if (isset($user_info['states_id'])) {
			$this->data['states_id'] = $user_info['states_id'];
		} else {
			$this->data['states_id'] = '';
		}
        
        if (isset($user_info['locality_id'])) {
			$this->data['locality_id'] = $user_info['locality_id'];
		} else {
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['mobile']) < 1) || (utf8_strlen($this->request->post['mobile']) > 32)) {
